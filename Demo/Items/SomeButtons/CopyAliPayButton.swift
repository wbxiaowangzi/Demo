...
class CopyAliPayButton: UIButton, CAAnimationDelegate {
    ...
    init(frame: CGRect, backgroundColor: UIColor) {
        ...
        self.setTitle("立即付款", for: UIControl.State.normal)
        self.setTitle("面容ID支付处理中", for: UIControl.State.disabled)
        self.addTarget(self, action: #selector(payAction), for: UIControl.Event.touchUpInside)
    }
    ...
    override func draw(_ rect: CGRect) {
        if !self.isEnabled {
            ...
            animation.fillMode = CAMediaTimingFillMode.forwards//动画执行完毕会停留在动画结束的位置
    ...
    public func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        ...
        self.setTitle("支付成功", for: UIControl.State.normal)
        self.isEnabled = true
    }
}
