...
extension AVPlayerVC: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return playerConfigArray.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PlayerListTableViewCell", for: indexPath)
        if playerConfigArray.count > indexPath.row {
            let playerConfig = playerConfigArray[indexPath.row]
            cell.textLabel?.text = playerConfig.title
        }
        cell.textLabel?.textColor = indexPath.row == currentPlayerConfigIndex ? .red : .gray
        return cell
    }
...
extension AVPlayerVC: HYPlayerCommonViewDelegate {
    /** 全屏状态改变*/
...
extension AVPlayerVC: HYPlayerCacheListViewDelegate {
    /** 确认删除缓存*/
